...
const updateFAQs=(priceData,capitalDomestic,capitalCommercial)=>{
  ...
  /* Smart FAQ for Cheapest - Don't repeat city name if same */
  if(cheapestDomestic.cities.length===1&&cheapestCommercial.cities.length===1&&cheapestDomestic.cities[0]===cheapestCommercial.cities[0]){
    updateEl("faqCheapest",`Currently, <strong>${cheapestDomestic.cities[0]}</strong> offers the lowest LPG prices in <strong>${STATE}</strong>. A domestic cylinder (14.2 kg) costs <strong>₹${cheapestDomestic.price.toFixed(2)}</strong>, while a commercial cylinder (19 kg) is priced at <strong>₹${cheapestCommercial.price.toFixed(2)}</strong>.`,false);
  }else{
    updateEl("faqCheapest",`The cheapest domestic cylinder (14.2 kg) in <strong>${STATE}</strong> is in <strong>${formatCityList(cheapestDomestic.cities)}</strong> at <strong>₹${cheapestDomestic.price.toFixed(2)}</strong>. For commercial cylinders (19 kg), the cheapest rate is <strong>₹${cheapestCommercial.price.toFixed(2)}</strong> in <strong>${formatCityList(cheapestCommercial.cities)}</strong>.`,false);
  }
  
  /* Smart FAQ for Expensive - Don't repeat city name if same */
  if(expensiveDomestic.cities.length===1&&expensiveCommercial.cities.length===1&&expensiveDomestic.cities[0]===expensiveCommercial.cities[0]){
    updateEl("faqExpensive",`Currently, <strong>${expensiveDomestic.cities[0]}</strong> has the highest LPG prices in <strong>${STATE}</strong>. A domestic cylinder (14.2 kg) costs <strong>₹${expensiveDomestic.price.toFixed(2)}</strong>, while a commercial cylinder (19 kg) is priced at <strong>₹${expensiveCommercial.price.toFixed(2)}</strong>.`,false);
  }else{
    updateEl("faqExpensive",`The most expensive domestic cylinder (14.2 kg) in <strong>${STATE}</strong> is in <strong>${formatCityList(expensiveDomestic.cities)}</strong> at <strong>₹${expensiveDomestic.price.toFixed(2)}</strong>. For commercial cylinders (19 kg), the highest price is <strong>₹${expensiveCommercial.price.toFixed(2)}</strong> in <strong>${formatCityList(expensiveCommercial.cities)}</strong>.`,false);
  }
...
const init=async()=>{
  try{
    ...
    /* Extract capital city prices BEFORE rendering table */
