/* Fuel Price Script v1.3 - Maximum Performance */
!function(){"use strict";
...
h=async()=>{try{
/* PARALLEL FETCH: All critical data at once */
const[g,h]=await Promise.all([
l(`https://cdn.frisqoo.com/${r}/${a}/${n}.json`),
l(`https://cdn.frisqoo.com/${r}/${a}.json`)
]);

/* PRIORITY 1: Update price box (most visible) */
...
/* PRIORITY 2: Table (chunked to 3 rows) */
setTimeout(()=>{
...
const s=a.slice(n,n+3),i=document.createDocumentFragment(),l=document.createElement("template");
l.innerHTML=s.map(e=>{const t=parseFloat(String(e[2]).replace(/[^0-9.-]/g,"")),{arrow:a,color:n}=u(t);return`<tr><td>${d(e[0])}</td><td>${e[1]}</td><td>${e[2]} <span style="font-weight:bold;color:${n}">${a}</span></td></tr>`}).join("");
i.appendChild(l.content);
0===n&&(e.innerHTML="");
e.appendChild(i);
n+=3;
n<a.length&&setTimeout(r,0);
};
r();
...
/* PRIORITY 3: Dropdowns (lazy loaded) */
requestIdleCallback(()=>{
Promise.all([
...
/* PRIORITY 4: Chart (lowest priority) */
requestIdleCallback(()=>{
