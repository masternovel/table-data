...
const formatFullMonthYear=d=>{const dt=new Date(d);const monthNames=["January","February","March","April","May","June","July","August","September","October","November","December"];return`${monthNames[dt.getMonth()]} ${dt.getFullYear()}`};
...
/* Parse only price without change - FIX FOR BUG 1 */
...
/* FAQ Comparison - FIXED FOR BUG 1 */
const updateComparisonFAQ=stateData=>{
  ...
  for(let i=1;i<stateData.length;i++){
    ...
    if(city===CITY)currentPrice=parsePrice(price);
    if(city===CAPITAL_CITY)capitalPrice=parsePrice(price);
...
const init=async()=>{
  try{
    ...
    if(historicalData?.length>1){
      ...
      /* Update FAQ - Last Month - FIXED FOR BUG 2 */
      if(historicalData.length>2){
        const lastMonth=parsePriceData(historicalData[2][1]);
        updateEl("faqLastMonthAnswer",`Last month in <strong>${formatFullMonthYear(historicalData[2][0])}</strong>, the domestic LPG (14.2 kg) price in <strong>${CITY}</strong> was <strong>${lastMonth.price}</strong> per cylinder.`,false);
      }
    }else{
