/* Fuel Price Script v1.1 - Optimized for Performance */
!function(){"use strict";
...
h=async()=>{try{
const b=Promise.all([l(`https://cdn.frisqoo.com/utilities/${r}/states.json`),l(`https://cdn.frisqoo.com/utilities/${r}/${a}.json`),l(`https://cdn.frisqoo.com/${r}/${a}.json`),l(`https://cdn.frisqoo.com/${r}/${a}/${n}.json`)]);
const[s,i,h,g]=await b;
setTimeout(()=>{const a=document.getElementById("stateSelect"),n=document.getElementById("citySelect");a.innerHTML=s.map(t=>`<option value="${t.name}" data-url="${t.url}"${t.name===e?" selected":""}>${t.name}</option>`).join(""),n.innerHTML=i.map(e=>`<option value="${e.name}" data-url="${e.url}"${e.name===t?" selected":""}>${e.name}</option>`).join(""),a.addEventListener("change",(function(){c(this)}),{passive:!0}),n.addEventListener("change",(function(){c(this)}),{passive:!0})},0);
setTimeout(()=>{const e=h.find((e,a)=>a>0&&e[0].trim().toLowerCase()===t.trim().toLowerCase());if(e){const[,e,t]=a,n=parseFloat(t.replace(/[^0-9.-]/g,"")),{arrow:r,color:s}=u(n);p("paraPrice",e),p("petrolPriceValue",`<span style="font-size:19px;font-weight:bold">${e}</span> <span style="font-size:14px">/ltr</span><span style="font-size:14px">(₹${Math.abs(n).toFixed(2)} <span style="font-weight:bold;color:${s}">${r}</span>)</span>`,!1),p("petrolPriceFAQ",`The ${o.toLowerCase()} price in ${t} is <strong>${e}</strong> per litre. Prices are updated daily at 6:00 AM IST.`,!1);const i=n>0?`has <strong style="color:red;">increased</strong> by ₹${Math.abs(n).toFixed(2)} since yesterday.`:n<0?`has <strong style="color:green;">decreased</strong> by ₹${Math.abs(n).toFixed(2)} since yesterday.`:`has <strong style="color:black;">remained the same</strong> compared to yesterday.`;p("faqTrendAnswer",`Compared to yesterday, the ${o.toLowerCase()} price in <strong>${t}</strong> ${i}`,!1)}},10);
setTimeout(()=>{const e=document.querySelector("#dataTable tbody");if(g?.length>1){const a=document.createDocumentFragment(),n=document.createElement("template");n.innerHTML=g.slice(1).map(e=>{const t=parseFloat(String(e[2]).replace(/[^0-9.-]/g,"")),{arrow:a,color:n}=u(t);return`<tr><td>${d(e[0])}</td><td>${e[1]}</td><td>${e[2]} <span style="font-weight:bold;color:${n}">${a}</span></td></tr>`}).join(""),a.appendChild(n.content),e.innerHTML="",e.appendChild(a),p("faqYesterdayAnswer",`Yesterday, on <strong>${d(g[1][0])}</strong>, the ${o.toLowerCase()} price in <strong>${t}</strong> was <strong>${g[1][1]}</strong> per litre.`,!1)}else e.innerHTML='<tr><td colspan="3" style="text-align:center!important;padding:20px;">Price history is currently unavailable.</td></tr>'},20);
requestIdleCallback(()=>{if(g?.length>1&&"undefined"!=typeof Chart){const e=g.slice(1).reverse();const a=h.find((e,a)=>a>0&&e[0].trim().toLowerCase()===t.trim().toLowerCase());a&&e.push([new Date().toISOString(),a[1],a[2]||"₹0.00"]);const n=e.map(e=>d(e[0]).split(",")[0]),r=e.map(e=>parseFloat(String(e[1]).replace(/[^0-9.]/g,"")));new Chart(document.getElementById("priceChart"),{type:"line",data:{labels:n,datasets:[{label:`${o} Price (₹/ltr)`,data:r,borderColor:"#4a90e2",backgroundColor:"rgba(74,144,226,0.1)",borderWidth:2,pointRadius:4,pointBackgroundColor:"#4a90e2",pointBorderColor:"#fff",pointBorderWidth:2,tension:.3,fill:!0}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!0,position:"top",labels:{font:{size:12},color:document.body.classList.contains("drK")?"#e0e0e0":"#333"}},tooltip:{backgroundColor:"rgba(0,0,0,0.8)",titleColor:"#fff",bodyColor:"#fff",borderColor:"#4a90e2",borderWidth:1,padding:10,displayColors:!1,callbacks:{label:function(e){return`Price: ₹${e.parsed.y.toFixed(2)}/ltr`}}}},scales:{x:{grid:{display:!1},ticks:{font:{size:11},color:document.body.classList.contains("drK")?"#b0b0b0":"#666"}},y:{beginAtZero:!1,grid:{color:document.body.classList.contains("drK")?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.05)"},ticks:{font:{size:11},color:document.body.classList.contains("drK")?"#b0b0b0":"#666",callback:function(e){return`₹${e.toFixed(2)}`}}}}}});}f(h),m(g)},{timeout:1e3})}catch(e){console.error("Init error:",e)}
requestIdleCallback(()=>{const e=document.querySelector(".pTtl > span");if(e){const t=e.innerText.split(" - Today")[0];e.innerHTML=`${t} - Today <span style="font-size:18px">(${currentDate})</span>`}l("https://www.frisqoo.com/feeds/posts/summary/-/Fuel?alt=json&max-results=5").then(e=>{const t=(e.feed?.entry||[]).map((e,t)=>`<p style="display:flex;align-items:center;margin:0"><span>${t+1}. </span><a href="${e.link.find(e=>"alternate"===e.rel).href}" target="_blank" style="margin-left:5px">${e.title.$t}</a></p>`).join(""),a=document.getElementById("related-posts");a&&(a.innerHTML=t)}).catch(()=>{})},{timeout:5e3})};
