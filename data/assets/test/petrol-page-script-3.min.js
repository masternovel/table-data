/* Fuel Price Script v1.1 - Performance Optimized - FIXED */
!function(){"use strict";
/* CFG Check */
...
/* Populate Dropdowns - RENAMED TO AVOID CONFLICT */
const b=document.getElementById("stateSelect"),y=document.getElementById("citySelect");b.innerHTML=s.map(t=>`<option value="${t.name}" data-url="${t.url}"${t.name===e?" selected":""}>${t.name}</option>`).join(""),y.innerHTML=i.map(e=>`<option value="${e.name}" data-url="${e.url}"${e.name===t?" selected":""}>${e.name}</option>`).join(""),b.addEventListener("change",(function(){c(this)}),{passive:!0}),y.addEventListener("change",(function(){c(this)}),{passive:!0});
/* Update Price Info */
const v=h.find((e,a)=>a>0&&e[0].trim().toLowerCase()===t.trim().toLowerCase());if(v){const[,e,t]=v,a=parseFloat(t.replace(/[^0-9.-]/g,"")),{arrow:n,color:r}=u(a);p("paraPrice",e),p("petrolPriceValue",`<span style="font-size:19px;font-weight:bold">${e}</span> <span style="font-size:14px">/ltr</span><span style="font-size:14px">(₹${Math.abs(a).toFixed(2)} <span style="font-weight:bold;color:${r}">${n}</span>)</span>`,!1),p("petrolPriceFAQ",`The ${o.toLowerCase()} price in ${t} is <strong>${e}</strong> per litre. Prices are updated daily at 6:00 AM IST.`,!1);const s=a>0?`has <strong style="color:red;">increased</strong> by ₹${Math.abs(a).toFixed(2)} since yesterday.`:a<0?`has <strong style="color:green;">decreased</strong> by ₹${Math.abs(a).toFixed(2)} since yesterday.`:`has <strong style="color:black;">remained the same</strong> compared to yesterday.`;p("faqTrendAnswer",`Compared to yesterday, the ${o.toLowerCase()} price in <strong>${t}</strong> ${s}`,!1)}
/* Update Table */
const w=document.querySelector("#dataTable tbody");if(g?.length>1){const e=g.slice(1).map(e=>{const t=parseFloat(String(e[2]).replace(/[^0-9.-]/g,"")),{arrow:a,color:n}=u(t);return`<tr><td>${d(e[0])}</td><td>${e[1]}</td><td>${e[2]} <span style="font-weight:bold;color:${n}">${a}</span></td></tr>`}).join("");w.innerHTML=e,p("faqYesterdayAnswer",`Yesterday, on <strong>${d(g[1][0])}</strong>, the ${o.toLowerCase()} price in <strong>${t}</strong> was <strong>${g[1][1]}</strong> per litre.`,!1)}else w.innerHTML='<tr><td colspan="3" style="text-align:center!important;padding:20px;">Price history is currently unavailable.</td></tr>';
/* Render Price Chart - DEFERRED */
setTimeout(()=>{if(g?.length>1&&"undefined"!=typeof Chart){const e=g.slice(1).reverse(),a=e.map(e=>d(e[0]).split(",")[0]),n=e.map(e=>parseFloat(String(e[1]).replace(/[^0-9.]/g,"")));requestAnimationFrame(()=>{new Chart(document.getElementById("priceChart"),{type:"line",data:{labels:a,datasets:[{label:`${o} Price (₹/ltr)`,data:n,borderColor:"#4a90e2",backgroundColor:"rgba(74,144,226,0.1)",borderWidth:2,pointRadius:4,pointBackgroundColor:"#4a90e2",pointBorderColor:"#fff",pointBorderWidth:2,tension:.3,fill:!0}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!0,position:"top",labels:{font:{size:12},color:document.body.classList.contains("drK")?"#e0e0e0":"#333"}},tooltip:{backgroundColor:"rgba(0,0,0,0.8)",titleColor:"#fff",bodyColor:"#fff",borderColor:"#4a90e2",borderWidth:1,padding:10,displayColors:!1,callbacks:{label:function(e){return`Price: ₹${e.parsed.y.toFixed(2)}/ltr`}}}},scales:{x:{grid:{display:!1},ticks:{font:{size:11},color:document.body.classList.contains("drK")?"#b0b0b0":"#666"}},y:{beginAtZero:!1,grid:{color:document.body.classList.contains("drK")?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.05)"},ticks:{font:{size:11},color:document.body.classList.contains("drK")?"#b0b0b0":"#666",callback:function(e){return`₹${e.toFixed(2)}`}}}}})})}},50);
/* Generate FAQ & Schema */
