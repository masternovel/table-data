/* Fuel Price Script v1.2 - Performance Optimized - ACTUALLY FIXED */
!function(){"use strict";
...
i={},
l=e=>i[e]||(i[e]=fetch(e).then(e=>e.ok?e.json():Promise.reject(e)).catch(err=>{console.error('Fetch error:',err);return null;})),
...
requestIdleCallback(()=>{
if(document.getElementById("frisqoo-custom-schema"))return;
document.querySelectorAll('script[type="application/ld+json"]:not([id])').forEach(e=>e.remove());
...
const[d,u]=o;
const p=parseFloat(String(u).replace(/[^0-9.-]/g,""));
d===t&&(a=u);
d===s&&(i=u);
if(!isNaN(p)){
...
// Populate Dropdowns - FIXED: Use direct innerHTML (faster and works!)
await new Promise(resolve=>{
const stateSelect=document.getElementById("stateSelect");
const citySelect=document.getElementById("citySelect");
if(stateSelect&&statesData&&Array.isArray(statesData)){
stateSelect.innerHTML=statesData.map(st=>`<option value="${st.name}" data-url="${st.url}"${st.name===e?" selected":""}>${st.name}</option>`).join("");
stateSelect.addEventListener("change",function(){c(this)},{passive:!0});
}
if(citySelect&&citiesData&&Array.isArray(citiesData)){
citySelect.innerHTML=citiesData.map(ct=>`<option value="${ct.name}" data-url="${ct.url}"${ct.name===t?" selected":""}>${ct.name}</option>`).join("");
citySelect.addEventListener("change",function(){c(this)},{passive:!0});
}
...
// Update Price Info
await new Promise(resolve=>{
if(stateCompareData&&Array.isArray(stateCompareData)){
const currentCity=stateCompareData.find((item,idx)=>idx>0&&item&&item[0]&&item[0].trim().toLowerCase()===t.trim().toLowerCase());
if(currentCity&&currentCity.length>=3){
const price=currentCity[1];
const change=currentCity[2];
...
// Update Table - FIXED: Use template element (proper way)
await new Promise(resolve=>{
const tbody=document.querySelector("#dataTable tbody");
if(tbody&&cityHistoryData&&Array.isArray(cityHistoryData)&&cityHistoryData.length>1){
const fragment=document.createDocumentFragment();
const template=document.createElement("template");
template.innerHTML=cityHistoryData.slice(1).map(row=>{
if(!row||row.length<3)return'';
const changeNum=parseFloat(String(row[2]).replace(/[^0-9.-]/g,""));
const{arrow,color}=u(changeNum);
return`<tr><td>${d(row[0])}</td><td>${row[1]}</td><td>${row[2]} <span style="font-weight:bold;color:${color}">${arrow}</span></td></tr>`;
}).join("");
fragment.appendChild(template.content);
tbody.innerHTML="";
tbody.appendChild(fragment);
if(cityHistoryData[1]&&cityHistoryData[1].length>=2){
...
// Render Chart
if(cityHistoryData&&Array.isArray(cityHistoryData)&&cityHistoryData.length>1&&"undefined"!=typeof Chart){
const currentCityData=stateCompareData&&Array.isArray(stateCompareData)?stateCompareData.find((item,idx)=>idx>0&&item&&item[0]&&item[0].trim().toLowerCase()===t.trim().toLowerCase()):null;
let chartInstance=null,historicalData=[],selectedDays=7;
const historicalURL=`https://cdn.frisqoo.com/historical/${r}/${a}/${n}.json`;
const formatDateShort=date=>{const d=new Date(date);return`${months[d.getMonth()]} ${String(d.getDate()).padStart(2,"0")}`};
const formatDateLong=date=>{const d=new Date(date);return`${months[d.getMonth()]} ${String(d.getDate()).padStart(2,"0")}, ${d.getFullYear()}`};
...
type:"line",
data:{
labels:filteredData.map(row=>formatDateShort(row[0])),
datasets:[{
label:`${o} Price (₹/ltr)`,
data:filteredData.map(row=>parseFloat(String(row[1]).replace(/[^0-9.]/g,""))),
borderColor:"#4a90e2",
...
plugins:{
legend:{display:!0,position:"top",labels:{font:{size:12},color:isDark?"#e0e0e0":"#333"}},
tooltip:{
...
displayColors:!1,
callbacks:{
title:tooltipItems=>formatDateLong(filteredData[tooltipItems[0].dataIndex][0]),
label:context=>{
const idx=context.dataIndex;
const price=filteredData[idx][1];
const change=parseFloat(filteredData[idx][2])||0;
...
scales:{
x:{grid:{display:!1},ticks:{font:{size:11},color:isDark?"#b0b0b0":"#666",maxRotation:45,minRotation:0,autoSkip:!0,maxTicksLimit:filteredData.length>90?12:15}},
y:{beginAtZero:!1,grid:{color:isDark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.05)"},ticks:{font:{size:11},color:isDark?"#b0b0b0":"#666",callback:val=>`₹${val.toFixed(2)}`}}
}
}
});
};
...
if(currentCityData&&currentCityData.length>=3&&allData.length>0){
...
const todayChange=currentCityData[2]?String(currentCityData[2]).replace(/[^0-9.-]/g,""):"0.00";
...
const posts=(data.feed.entry||[]).map((entry,idx)=>`<p style="display:flex;align-items:center;margin:0"><span>${idx+1}. </span><a href="${entry.link.find(lnk=>"alternate"===lnk.rel).href}" target="_blank" style="margin-left:5px">${entry.title.$t}</a></p>`).join("");
...
document.addEventListener("DOMContentLoaded",h,{passive:!0,once:!0});
}else{
h();
}
...
if(document.readyState==="loading"){
document.addEventListener("DOMContentLoaded",addRadioHandlers,{passive:!0,once:!0});
}else{
addRadioHandlers();
}
