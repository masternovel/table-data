  ...
  try{
    ...
    if(currentData?.length>1){
      ...
      updateEl("lpgPriceValue",
        `<span style="font-size:19px;font-weight:bold">${domesticPrice}</span> <span style="font-size:14px">/cylinder</span>`,
        false
      );
    }
    ...
    if(currentData?.length>1){
      ...
      template.innerHTML=currentData.slice(1).map(row=>{
        ...
        return`<tr><td>${type}</td><td style="text-align:right">${price}</td><td style="text-align:right">${change} <span style="font-weight:bold;color:${color}">${arrow}</span></td></tr>`;
      }).join("");
    ...
    if(historicalData?.length>1){
      ...
      template.innerHTML=historicalData.slice(1,11).map(row=>{
        ...
        return`<tr><td>${formatMonthYear(date)}</td><td style="text-align:right">${domestic.price} <span style="font-size:13px">(${domestic.change.toFixed(2)} <span style="font-weight:bold;color:${dColor}">${dArrow}</span>)</span></td><td style="text-align:right">${commercial.price} <span style="font-size:13px">(${commercial.change.toFixed(2)} <span style="font-weight:bold;color:${cColor}">${cArrow}</span>)</span></td></tr>`;
      }).join("");
