/* Fuel Price Script v1.1 - Performance Optimized */
!function(){"use strict";
/* CFG Check */
...
/* Populate Dropdowns */
const m=()=>{const a=document.getElementById("stateSelect"),n=document.getElementById("citySelect");a.innerHTML=s.map(t=>`<option value="${t.name}" data-url="${t.url}"${t.name===e?" selected":""}>${t.name}</option>`).join(""),n.innerHTML=i.map(e=>`<option value="${e.name}" data-url="${e.url}"${e.name===t?" selected":""}>${e.name}</option>`).join(""),a.addEventListener("change",(function(){c(this)}),{passive:!0}),n.addEventListener("change",(function(){c(this)}),{passive:!0})};m();
/* Update Price Info */
const y=h.find((e,a)=>a>0&&e[0].trim().toLowerCase()===t.trim().toLowerCase());if(y){const[,e,t]=y,a=parseFloat(t.replace(/[^0-9.-]/g,"")),{arrow:n,color:r}=u(a);p("paraPrice",e),p("petrolPriceValue",`<span style="font-size:19px;font-weight:bold">${e}</span> <span style="font-size:14px">/ltr</span><span style="font-size:14px">(₹${Math.abs(a).toFixed(2)} <span style="font-weight:bold;color:${r}">${n}</span>)</span>`,!1),p("petrolPriceFAQ",`The ${o.toLowerCase()} price in ${t} is <strong>${e}</strong> per litre. Prices are updated daily at 6:00 AM IST.`,!1);const s=a>0?`has <strong style="color:red;">increased</strong> by ₹${Math.abs(a).toFixed(2)} since yesterday.`:a<0?`has <strong style="color:green;">decreased</strong> by ₹${Math.abs(a).toFixed(2)} since yesterday.`:`has <strong style="color:black;">remained the same</strong> compared to yesterday.`;p("faqTrendAnswer",`Compared to yesterday, the ${o.toLowerCase()} price in <strong>${t}</strong> ${s}`,!1)}
/* Update Table */
const b=document.querySelector("#dataTable tbody");if(g?.length>1){const e=g.slice(1).map(e=>{const t=parseFloat(String(e[2]).replace(/[^0-9.-]/g,"")),{arrow:a,color:n}=u(t);return`<tr><td>${d(e[0])}</td><td>${e[1]}</td><td>${e[2]} <span style="font-weight:bold;color:${n}">${a}</span></td></tr>`}).join("");b.innerHTML=e,p("faqYesterdayAnswer",`Yesterday, on <strong>${d(g[1][0])}</strong>, the ${o.toLowerCase()} price in <strong>${t}</strong> was <strong>${g[1][1]}</strong> per litre.`,!1)}else b.innerHTML='<tr><td colspan="3" style="text-align:center!important;padding:20px;">Price history is currently unavailable.</td></tr>';
